...
### 连接方面
1. HTTP 1.1 支持持久连接，每个请求可以服用同一个TCP 连接，但同一时刻只能处理一个请求，下一个请求需要等待上一个请求完成后才能发送。
2. HTTP 2.0 支持多路复用，多个请求可以在同一个连接上同时发送和接受，不会因为某个请求的阻塞而影响其他请求，大大提高了连接的利用率和性能

### 数据传输
1. HTTP 1.1 数据以文本的形式传输，头部信息不压缩，会随着每个请求和响应重复发送，增加了传输的数据量和带宽占用
2. HTTP 2.0 支持二进制数据传输，头部信息压缩，减少了传输的数据量和带宽占用，也提高了安全性

### 请求优先级
1. HTTP 1.1 不支持请求优先级，所有请求都被视为相等重要，服务器会按顺序处理
2. HTTP 2.0 支持请求优先级，客户端可以为每个请求设置优先级，服务器会根据优先级处理请求

### 服务器推送

1. HTTP 1.1 不支持服务器推送，服务器只能根据客户端的请求，返回对应的响应
2. HTTP 2.0 支持服务器推送，服务器可以根据客户端的请求，主动推送相关的资源，提高用户体验 提高页面加载速度
