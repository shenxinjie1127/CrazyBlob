...
- **拆分模块** vue3 注重模块上拆分，各个模块之间耦合度低，模块可以独立使用。
- **组合式API** 通过构建工具 Tree-shaking机制实现按需引入，减少用户打包后体积。
- **扩展更方便** Vue3允许自定义渲染器，扩展能力强。扩展更方便。
- **采用RFC规范** 使用RFC来确保改动和设计都是经过Vue3团队探讨并得到确认，用户可以了解一个功能采用或废弃的前因后果。
